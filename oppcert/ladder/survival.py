#!/usr/bin/env python3
"""
oppcert/ladder/survival.py -- measure S(100) on the EXACT arm the mechanism section describes.

PAPER: supplementary Sec. C. Prefetched-object survival on the scored arm, S(100) = 0.984 against the
baseline's 1.000 -- the measurement that rules out eviction as the cause and withdraws the
endogenous-slack claim.
...
falls toward 0.05"). But `oppcert/ladder/r6_sweep.py --survcheck` does NOT measure that arm. It instantiates r8's
...
oppcert.ladder.r6_placement.run() uses, replays it with record_survival=True, and reports S(100) under three estimators
...
same policy, same budget, same evictor, same trace prefix.
    ...
    python -m oppcert.ladder.survival --trace data/wiki_2019t.oracleGeneral --pred markov2 --tau 0.05 --k 1 \
        --haz haz/wiki.npz --wide-k 32 --gamma 0.1
    python -m oppcert.ladder.survival --trace data/cluster50.sample10.oracleGeneral --pred markov3 --tau 0.06 \
...
from oppcert.ladder.r6_placement import CausalTimedCover
from oppcert.sim.prefetch import PFCache
...
def main():
    ...
    # identical construction to oppcert.ladder.r6_placement.run()'s model arm
