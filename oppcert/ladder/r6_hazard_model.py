#!/usr/bin/env python3
"""
oppcert/ladder/r6_hazard_model.py -- Necessity Ladder rung (ii), part A: the CAUSAL hazard model + eviction-survival

PAPER: Stage 2. The causal hazard model r6's model arm consumes, plus its learnability gate (rank
correlation 0.60 / 0.43, never-reused AUC 0.92 / 0.81).
curve that the HJS-L scheduler (oppcert/unreported/hjsl_scheduler.py) consumes, plus the F2 learnability gate.
...
F2 GATE (pre-registered, on the held-out EVAL half):
  ...
  scheduler, and that trace drops out of the capture claim. Report it.
    ...
    python -m oppcert.ladder.r6_hazard_model --trace data/wiki_2019t.oracleGeneral --pred markov2 --tau 0.05 --k 1 \
        --out haz/wiki.npz
    python -m oppcert.ladder.r6_hazard_model --selftest        # runs on a synthetic trace, no data needed
"""
...
from oppcert.ladder.r2_arbitration import capture_ctx, conf_lookup                # reuse the exact confidence lookup
from oppcert.sim.prefetch import PFCache, oracle_next
from oppcert.instrument.gates import prep

# ---- fixed binning (shared with oppcert/unreported/hjsl_scheduler.py via the saved npz) --------------------------------
...
def selftest():
    ...
    print("  [oppcert.ladder.r6_hazard_model selftest] completed without error\n")
