#!/usr/bin/env python3
"""oppcert/ladder/r7_cgp.py -- Corroboration-Gated Prefetch (CGP): the CONSTRUCTIVE rung.

PAPER: Stage 2 rung r7, Corroboration-Gated Prefetch. -7.60 / -5.39 against the tuned baseline; +0.98 /
+1.01 against the capped-baseline control, missing its pre-registered 2.5-point gate. Pre-registered in
       prereg/r7_cgp.md.
...
PRE-REGISTERED GATE (fixed before any run; see prereg/r7_cgp.md):
  ...
  FAIL  -> ladder rung 6; the negative hardens (even coverage/volume decoupling fails).
    ...
    python -m oppcert.ladder.r7_cgp --trace data/wiki_2019t.oracleGeneral        --pred markov2 --tau 0.05 --k 1 \
        --wide-k 32 --wide-top-m 32 --f5-ref 10.41 --verbose
    python -m oppcert.ladder.r7_cgp --trace data/cluster50.sample10.oracleGeneral --pred markov3 --tau 0.06 --k 1 \
        --wide-k 16 --f5-ref 11.42 --verbose
    python -m oppcert.ladder.r7_cgp --selftest
"""
...
def run(trace, cap, pos, szs, args):
    ...
    if args.f5_ref is not None:
        print(f"  F5 REF    corridor {args.f5_ref:+.2f} pts   (clairvoyant coverage+JIT ceiling, from docs/results_ledger.md 6)")
    print(f"  {'-'*80}")
...
def selftest():
    ...
    print("  [oppcert.ladder.r7_cgp selftest] completed without error\n")
...
def main():
    ...
    ap.add_argument("--f5-ref", type=float, default=None,
                    help="F5 corridor for %%-capture context (wiki 10.41, cluster50 11.42 from docs/results_ledger.md)")
