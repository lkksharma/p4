#!/usr/bin/env python3
"""oppcert/ladder/r4_coverage.py -- Necessity Ladder rung (i.5): the F5 COVERAGE-CEILING gate.

PAPER: Stage 2 rung r4, coverage (internal name: F5). Wide emission + clairvoyant JIT insertion: +10.41 /
+11.42. The corridor is reachable in principle.
...
PRE-REGISTERED FORK (checklist point 1, fixed BEFORE this run):
  ...
  F5 ~= bar                                      -> width does not help with this predictor ->
    ...
    python -m oppcert.ladder.r4_coverage --trace data/wiki_2019t.oracleGeneral        --pred markov2 --tau 0.05 --k 1
    python -m oppcert.ladder.r4_coverage --trace data/cluster50.sample10.oracleGeneral --pred markov3 --tau 0.06 --k 1
    python -m oppcert.ladder.r4_coverage --selftest
"""
...
from oppcert.sim.prefetch import PFCache, Prescient
from oppcert.instrument.gates import KS, prep


def block_ci(diff, blocks, resamples, seed):
    """Paired moving-block bootstrap -- same convention as oppcert.ladder.r1_r3_timing/oppcert.unreported.hjsl_scheduler."""
    R = len(diff)
    L = R // blocks
...
def selftest():
    ...
    print("  [oppcert.ladder.r4_coverage selftest] completed without error\n")
