#!/usr/bin/env python3
"""oppcert/ladder/r6_sweep.py -- closing the two open holes in Stage 2: the r6 model sweep, and volume control.

PAPER: Stage 2 robustness for r6 (internal name: r8). Model sweep (retrain on the wide stream, rank
correlation 0.68, no capture gained) and the volume control (1.02M vs 0.98M fetches on wiki).
...
------------------------
  ...
  capture rises but plateaus below the bar     -> the quantitative statement: the volume needed for
    ...
    python -m oppcert.ladder.r6_sweep --trace data/wiki_2019t.oracleGeneral --pred markov2 --tau 0.05 --k 1 \
        --haz haz/wiki.npz --sweep volume
    python -m oppcert.ladder.r6_sweep --selftest
"""
...
from oppcert.ladder.r6_placement import _quantile
from oppcert.ladder.r6_hazard_model import N_CONF, N_FREQ, N_REC, conf_bin, freq_bin, recency_bin, run as haz_run
...
def selftest():
    ...
    print("  [oppcert.ladder.r6_sweep selftest] completed without error\n")
...
def main():
    ...
    ap.add_argument("--haz", help="hazard npz from oppcert/ladder/r6_hazard_model.py")
