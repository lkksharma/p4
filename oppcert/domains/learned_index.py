#!/usr/bin/env python3
"""
oppcert/domains/learned_index.py -- the Instrument in a SECOND, non-caching domain: learned indexes.

PAPER: Sec. 6, learned indexes -- verdict BUILD on books, 1 of 5 real key sets (Table 4). The positive
control: the same procedure, unchanged, returning a green light.
...
Usage
-----
    python -m oppcert.domains.learned_index                       # all distributions, N = 1,000,000
    python -m oppcert.domains.learned_index --n 2000000 --leaves 20000
"""
...
def keys_from_trace(path, limit):
    ...
    from oppcert.sim.trace import load_oracle_general
