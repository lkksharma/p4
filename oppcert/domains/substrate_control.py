#!/usr/bin/env python3
"""
oppcert/domains/substrate_control.py -- the EVICTION green-light: the Instrument's positive control.

PAPER: Sec. 6, the substrate-competence control. LRB-like reuse-distance evictor and Baleen-like
admission classifier, scored against the eviction-management corridor (+11.55 / +12.50 / +12.75). Needs
       scikit-learn.
...
Usage
-----
    python -m oppcert.domains.substrate_control --selftest                       # construction check on SYNTH
    python -m oppcert.domains.substrate_control --trace data/wiki_2019t.oracleGeneral --limit 2000000 --tqdm
    python -m oppcert.domains.substrate_control --trace SYNTH                     # quick positive-control smoke
"""
...
from oppcert.sim.evict import BeladyEvictor, LRUEvictor, S3FIFOEvictor
...
def synth_positive(n_req=150_000, n_hot=3000, hot_zipf=1.05, p_hot=0.5, cold_size=4, seed=0):
    ...
    from oppcert.sim.trace import compute_next_access
...
def block_ci(diff, blocks=1000, resamples=10000, seed=0):
    """Paired moving-block bootstrap 95% CI (same convention as oppcert/sim/bootstrap.py). diff is the
...
def selftest():
    ...
    from oppcert.sim.trace import footprint_bytes, synth_trace
    from oppcert.sim.prefetch import build_obj_positions, build_obj_sizes
    ...
    print(f"[oppcert.domains.substrate_control selftest] PASS -- replay==PFCache for S3-FIFO and Belady; "
...
def learned_demo(seed=0):
    ...
    from oppcert.sim.trace import footprint_bytes
    from oppcert.sim.prefetch import build_obj_positions, build_obj_sizes
...
def run(trace, cap, pos, szs, args):
    ...
    # claim removed from oppcert/domains/learned_index.py; a claim about the data must be computed from the data.
    if cap_pt > 0.05:
