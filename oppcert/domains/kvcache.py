#!/usr/bin/env python3
"""oppcert/domains/kvcache.py -- PHANTOM HEADROOM, ported to LLM KV-CACHE SERVING (Mooncake traces).

PAPER: Sec. 6, LLM KV-cache serving -- verdict NO CORRIDOR. -0.56 / -1.87 against the tuned baseline;
+2.98 / +2.14 against the capped-baseline control, under an 8-point gate.
...
PRE-REGISTERED (fixed before the first real-trace run; bars inherited unchanged from the paper):
  ...
  F2-lite   use-lag learnability on the bar's own emission stream: Spearman >= 0.20 AND
            never-AUC >= 0.60 (oppcert.ladder.r6_hazard_model, three-way split, unchanged).
...
ONE COMMAND -> ONE TABLE (bar, ceiling, corridor, CI, gate, W2b signal, F2, S(100)) + fork.
    ...
    python -u -m oppcert.domains.kvcache --trace data/mooncake_conv.jsonl --verbose
    python -u -m oppcert.domains.kvcache --selftest
...
from oppcert.sim.trace import compute_next_access, footprint_bytes
from oppcert.sim.invariants import gate_gap, gate_prefetch_signal
from oppcert.sim.prefetch import (Markov1, Markov2, PFCache, Prescient, build_obj_positions,
                         build_obj_sizes)
from oppcert.instrument.bars import Markov3, STRONG_TAUS
from oppcert.instrument.gates import KS
from oppcert.ladder import r6_hazard_model
...
def run(trace, args, name):
    ...
    fm = {}
    r6_hazard_model.run(trace, cap, pos, szs, h, metrics=fm)
...
def main():
    ...
    if args.selftest:
        ...
        print("  [oppcert.domains.kvcache selftest] completed without error\n")
        return
    if not args.trace:
