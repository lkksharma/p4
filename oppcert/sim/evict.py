#!/usr/bin/env python3
"""
oppcert/sim/evict.py -- pluggable eviction policies.

PAPER: replay substrate. Pluggable evictors; S3-FIFO is the one held fixed throughout Stage 2.

The cache (oppcert.sim.prefetch.PFCache) owns bytes, hit accounting and the prefetch hook; an Evictor owns
...
MUST be parity-checked against libCacheSim's S3FIFO before use (oppcert.sim.invariants --parity-s3fifo);
...
class BeladyEvictor:
    ...
    def _set(self, o, t):
        from oppcert.sim.prefetch import oracle_next
