#!/usr/bin/env python3
"""
oppcert/sim/prefetch.py -- prefetch-aware simulation + the T1 skeleton (guide W2 deliverable).

PAPER: replay substrate. Prefetch-aware simulation and the token bucket that meters every oracle arm to
the baseline's measured byte budget (Sec. 1 commitment two).
...
class PFCache:
    ...
    def run(self, trace, warmup_frac=0.05, cold_train_frac=None, return_hits=False,
        ...
        from oppcert.sim.evict import make_evictor
...
def selftest():
    ...
    print("[oppcert.sim.prefetch selftest] PASS -- prefetch accounting contract holds "
