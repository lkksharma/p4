#!/usr/bin/env python3
"""
oppcert/instrument/lstm_bar.py -- train a next-item LSTM on a trace PREFIX and dump per-position

PAPER: Stage 1. The LSTM decoupled bar -- the kill-test behind 'the LSTM never recovers a corridor' (Sec.
4). Needs torch.
top-M predictions for the WHOLE trace, for replay by oppcert/instrument/bars.py's LSTM arm.
...
Typical (DGX, one GPU):  ~5-10 min train + ~1-2 min dump per trace.
    python -m oppcert.instrument.lstm_bar --trace data/wiki_2019t.oracleGeneral --limit 2000000 \
