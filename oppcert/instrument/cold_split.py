#!/usr/bin/env python3
"""
oppcert/instrument/cold_split.py -- cold-miss decomposition of an already-measured corridor, WITHOUT

PAPER: Stage 1. The reachability restriction: drops objects no history-based predictor could name. Source
of the 14-87% gross-corridor deflation in Table 2.
...
Example (wiki, bar from logs/strongbar_wiki_2019t.log):
    python -m oppcert.instrument.cold_split --trace data/wiki_2019t.oracleGeneral --limit 2000000 \
...
from oppcert.sim.prefetch import Markov1, Markov2, PFCache, Prescient
...
# Online-updating variants (prereg/online_predictor.md). Additive only: the frozen entries above are
# the pre-registered arms and are left exactly as they were.
from oppcert.instrument.online_bars import ONLINE_PREDS as _ONLINE
