#!/usr/bin/env python3
"""
oppcert/instrument/gates.py -- the two gates that decide P4 after the joint thesis died on MSR.

PAPER: Stage 1. --gate a is the Instrument itself (learnable corridor, Table 2); --gate b is the eviction
x prefetch 2x2 behind 'eviction and prefetching are substitutes' (Sec. 4).
...
Usage:
    python -m oppcert.instrument.gates --trace data/msr_proj_0.oracleGeneral --gate a
    python -m oppcert.instrument.gates --trace data/msr_proj_0.oracleGeneral \
                       --trace data/wiki2019.oracleGeneral \
                       --trace data/twitter_c52.oracleGeneral --gate b
    python -m oppcert.instrument.gates --trace ... --gate both        # A on trace 1, B on all
"""
...
from oppcert.sim.trace import footprint_bytes, load_oracle_general, synth_trace
from oppcert.sim.prefetch import (Markov1, Markov2, NoPrefetch, PFCache, Prescient, build_obj_positions,
...
def bw_rate(trace, cap, pos, szs, mk):
    ...
    convention oppcert/unreported/t1_mechanism.py uses, as a RATE (bytes/request), not a total budget."""
...
def gate_a(trace, cap, pos, szs, args, preds=None):
    ...
    Callers (oppcert/instrument/bars.py) pass supersets -- the bar is the max over everything offered.
...
def gate_b_one(trace, cap, pos, szs, args):
    """The identical 2x2 from oppcert/unreported/t1_mechanism.py: {S3-FIFO, Belady} x {none, Markov-1, Prescient@BW}."""
