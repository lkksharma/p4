#!/usr/bin/env python3
"""
oppcert/instrument/bars.py -- the STRONGER decoupled bars the corridor must survive before we call it real.

PAPER: Stage 1. The decoupled predictor family Gate A sweeps to fix the adversarially tuned baseline.
...
Day 0 (no GPU needed):
    python -m oppcert.instrument.bars --trace data/wiki_2019t.oracleGeneral --limit 2000000
Day 1 (after oppcert/instrument/lstm_bar.py dumps predictions):
    python -m oppcert.instrument.bars --trace data/wiki_2019t.oracleGeneral --limit 2000000 \
...
from oppcert.sim.prefetch import Markov1, Markov2, _MarkovBase, _rank
from oppcert.instrument.gates import gate_a, prep, selftest
...
class LSTMTopK(_MarkovBase):
    ...
    so oppcert/instrument/lstm_bar.py runs the model over the trace once on GPU and dumps, for every
...
def main():
    ...
    ap.add_argument("--lstm-preds", default=None,
                    help="npz from oppcert/instrument/lstm_bar.py; adds the LSTM arm")
    args = ap.parse_args()
