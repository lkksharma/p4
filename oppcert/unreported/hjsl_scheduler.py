#!/usr/bin/env python3
"""
oppcert/unreported/hjsl_scheduler.py -- Necessity Ladder rung (ii), part B: the HJS-L closed-form prefetch SCHEDULER, and

PAPER: NOT REPORTED. The HJS-L closed-form hazard-priced scheduler -- a dead arm superseded by r6. See
       oppcert/unreported/README.md.
...
predicted use-lag distribution (oppcert/ladder/r6_hazard_model.py) and the measured eviction-survival curve S(delta),
...
timing-only ceiling on the predictor's own candidates (oppcert/ladder/r1_r3_timing.py). Reporting against F1, not the v3
...
whose F2 gate passed and whose F1 >= 8.
    ...
    python -m oppcert.unreported.hjsl_scheduler --trace data/wiki_2019t.oracleGeneral --pred markov2 --tau 0.05 --k 1 \
        --haz haz/wiki.npz
...
from oppcert.ladder.r1_r3_timing import emit_vocab
from oppcert.ladder.r2_arbitration import capture_ctx, conf_lookup
from oppcert.ladder.r6_hazard_model import (LAG_EDGES, NEVER_BIN, N_CONF, N_FREQ, N_REC, conf_bin, freq_bin,
...
def run(trace, cap, pos, szs, haz, args):
    ...
    # F1 timing-only ceiling (the honest denominator) -- vocab oracle, same as oppcert/ladder/r1_r3_timing.py
...
def selftest():
    ...
    print("  [oppcert.unreported.hjsl_scheduler selftest] completed without error\n")
...
def main():
    ...
    ap.add_argument("--haz", help="hazard+survival .npz from oppcert/ladder/r6_hazard_model.py")
