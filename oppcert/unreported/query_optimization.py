#!/usr/bin/env python3
"""oppcert/unreported/query_optimization.py -- Opportunity Certification in a fifth domain: query optimization.

PAPER: NOT REPORTED. Opportunity Certification on the Join Order Benchmark, a domain outside this paper's
scope. See oppcert/unreported/README.md.

Runs the protocol pre-registered in prereg/unreported_query_optimization.md end to end, in one command, on the Join
...
----------------
    ...
    reason is recorded in prereg/unreported_query_optimization.md 10.

    corridor = tuned baseline latency - reachable ceiling latency, as a % of tuned baseline.

PRE-REGISTERED BAR (prereg/unreported_query_optimization.md 5). Both conditions must hold:
...
the measured corridor is an upper bound, so a negative verdict is conservative.
    ...
    python -m oppcert.unreported.query_optimization --selftest                 # pure-python logic, no database required
    python -m oppcert.unreported.query_optimization --setup --db imdb          # verify schema, statistics, extension availability
    python -m oppcert.unreported.query_optimization --run --db imdb --queries job/ --out logs/job.json
"""
...
# ---- pre-registered constants; read from prereg/unreported_query_optimization.md, never tuned to an outcome ----
...
# Reachability split, amended 2026-07-26 BEFORE any measurement (prereg/unreported_query_optimization.md 10):
...
def run(args):
    ...
    print(f"  {len(queries)} queries | bar: corridor >= {BAR_CORRIDOR_PCT:.0f}% AND > config span "
          f"(pre-registered, prereg/unreported_query_optimization.md)")
...
def selftest():
    ...
    print("  [oppcert.unreported.query_optimization selftest] PASS -- parsing, predicate split, join graph, connected-subset "
