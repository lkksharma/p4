#!/usr/bin/env python3
"""
p4_online.py -- online-updating variants of the Markov bars (SPEC_online_prereg.md).

The frozen predictors in p4_prefetch.py / p4_strongbar.py train on the trace prefix and stop.
...
request would dominate the replay.
    ...
    python p4_online.py --leaktest            # I1 + I4, on synthetic data
"""
...
from p4_prefetch import Markov1, Markov2, _MarkovBase
...
def _counters_from_prefix(trace, train_frac, window, order):
    ...
    Mirrors the training loops in p4_prefetch/p4_strongbar exactly, including the `j < cut`
...
def leaktest():
    ...
    print("\n  " + ("ALL INVARIANTS PASS -- safe to run the pre-registered arms"
                    if ok else "INVARIANT FAILURE -- per SPEC_online_prereg.md the run is aborted"))
    return 0 if ok else 1


def main():
    ap = argparse.ArgumentParser(description="online-updating Markov bars (SPEC_online_prereg.md)")
