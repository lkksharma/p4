...
### 3.4 F5 — confirmed, both traces (`oppcert/ladder/r4_coverage.py`, new this session)
...
### 3.5 Policy 1 — dead, but the audit trail matters (`oppcert/ladder/r5_emission.py`, new this session)
...
### 3.6 F7 — the decisive gate, running now (`oppcert/ladder/r6_placement.py`, new this session)
...
3. **Branch on the final verdict:**
   - PASS anywhere → design and build the full DEFER-for-wide policy (reuse `oppcert/ladder/r6_placement.py`'s
