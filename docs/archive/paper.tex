...
\item \textbf{Causal insertion timing fails.} A hazard model of time-to-next-use, though it passes the standard rank-learnability gate, cannot place insertions: it leaves the policy 26 to 40 points \emph{below} the baseline, and retraining it to a higher rank correlation ($\rho{=}0.68$) does not help (\S\ref{sec:f7}).
\item \textbf{The constructive escape also fails.} A policy built specifically to buy coverage without volume (a byte-free wide watchlist plus event-triggered firing under the bar's exact byte budget) does not beat the tuned bar, because the corroboration signal is statistically redundant with the predictor's own confidence: gating on it raises no precision (\S\ref{sec:cgp}).
\end{itemize}
...
\node[verd,fill=dead,text=white,right=of f7] (v7) {no: $-40$/$-26$ pts, below bar};
...
The hazard model \emph{passes} the standard learnability gate that would normally justify building on it: Spearman rank correlation between predicted and true lags of $+0.60$ (Wikipedia) and $+0.43$ (\texttt{cluster50}), never-used AUC 0.92 and 0.81. The construction check passes: the oracle arm reproduces the F5 ceiling ($+9.69$ $[+8.07,+11.33]$, 93.1\% of F5 on Wikipedia; $+11.69$ $[+10.23,+13.18]$, 102.4\% on \texttt{cluster50}; both 100\% on-time). The model arm nevertheless fails catastrophically. Its corridor is $-40.1$ points $[-42.7,-37.4]$ on Wikipedia and $-26.0$ $[-27.9,-24.1]$ on \texttt{cluster50}: not merely short of the $50\%$-of-ceiling gate but far \emph{below the baseline itself}. Every funded object is one F5 also funds (clairvoyant selection), yet precision collapses to $0.05$--$0.06$ and the policy issues $1.0$--$1.5$M fetches against the bar's $1.0$M, flooding the cache. The on-time rate is only $58$--$75\%$, and even the on-time fetches are mostly evicted before use, so two failure modes pincer the policy at once (\S\ref{sec:slack}).

The pre-registered fair-shot fallback retrains the hazard model on the wide emission stream it is evaluated on. This \emph{raises} its rank correlation (Wikipedia $\rho{:}\,0.60{\to}0.68$, AUC $0.94$; \texttt{cluster50} $\rho{=}0.40$, AUC $0.82$; both still clear the F2 gate), yet capture is unchanged: $-40.4$ and $-25.2$ points. A better-fit forecaster does not place insertions any better. The claim the numbers support is therefore precise and doubly confirmed: \emph{causal just-in-time warming under the baseline's bandwidth budget is unreachable, and improving forecast rank accuracy does not change that}.
...
\textcolor{dead}{clairvoyant, wide} & causal hazard & F7 model & $-40.06$ & $-26.01$ \\
causal (armed wide) & causal (fire event) & CGP & $-7.60$ & $-5.39$ \\
...
Why does a hazard model that ranks lags well ($\rho\approx0.6$) fail so completely at placement? The answer is a feedback loop the gates expose directly. Under the \emph{bar's} modest fetch load, prefetched objects essentially never leave the cache before use ($S(100){=}1.0$: survival past 100 requests is certain), so a timing model would seem to enjoy enormous slack. But slack is not a property of the workload; it is a property of the policy's own behavior. A wide-net causal policy that mistimes fetches issues far more of them (precision collapses toward 0.05), and that volume churns the cache, which collapses the survival curve, which shrinks the residence window every subsequent fetch must hit, which makes mistiming more costly still. The collapse is measured directly: under the wide emission load the survival curve falls from $S(100){=}1.0$ to $0.055$ on Wikipedia and $0.135$ on \texttt{cluster50}, with median residence dropping to 54 and 22 requests. The window a timing model must hit is not the workload's; it is the one the policy's own errors have already narrowed. Clairvoyant insertion needs zero slack (arrive one request early); a log-binned hazard model has multiplicative error on heavy-tailed lag distributions, so at any hedging quantile it simultaneously overshoots some uses (LATE) and arrives so early on others that its own churn evicts the object before use. The two failure modes pincer the policy at every $\gamma$.

The general lesson: \textbf{rank-learnable is not placeable}. A learnability gate certifies that a signal orders correctly; an insertion decision needs the signal to land inside a window whose width the decision itself determines. The fallback experiment isolates this cleanly: retraining the hazard model on the wide stream raised its rank correlation from 0.60 to 0.68 and left its capture unchanged (a fraction of a point, still $-40$). More rank accuracy bought no placement. Headroom studies and learned-prefetcher evaluations do not currently distinguish the two properties, and the F2-pass/F7-fail pair measured here is, to current knowledge, the first direct demonstration that the gap between them can be total.

\subsection{The constructive escape, and why it also fails}\label{sec:cgp}
Endogenous slack has a constructive dual: if fetch volume is what collapses the residence window, then a policy that buys coverage \emph{without} buying volume should escape. Corroboration-Gated Prefetch (CGP) is the minimal such design. A wide predictor (the F5 fanout) \emph{arms} candidates into a byte-free watchlist, costing no fetch and no cache slot; a tight predictor then \emph{fires} an actual insertion only on its own event, scored $s(X){=}\mathrm{conf}_{\mathrm{fire}}(X)\,(1{+}\beta\,\mathrm{armed}(X))$ and funded under the bar's exact byte budget. Coverage comes from the free arm; byte volume is pinned at the bar's, so the survival curve cannot collapse. CGP is separable (escapes F4$'$), fetches a set the bar never offers (escapes F1-stream), never fetches at wide emission (escapes Policy 1), and binds insertion to an observed event rather than a forecast time (escapes F7). Its clairvoyant limit is F5, so its construction check is the already-validated ceiling. The gate, pre-registered before the run: capture $\geq{+}2.5$ points over the bar, bootstrap CI clear of zero, on both traces.

It fails: $-7.60$ $[-8.11,-7.08]$ on Wikipedia and $-5.39$ $[-5.89,-4.89]$ on \texttt{cluster50}, far below the bar. The reason is measured and specific. Raising $\beta$ shifts the funded set from 43\% to 69\% armed while its precision does not move at all (0.269 to 0.268 on \texttt{cluster50}): armed and un-armed fires convert at the same rate, so the arm lift over the fire-only ablation is $+0.18$ and $-0.04$. \textbf{The corroboration signal is statistically redundant with the predictor's own confidence.} On these workloads, ``this object was plausible over a long horizon'' adds nothing to ``this object is likely next,'' so there is no causal way to widen coverage without also widening the low-precision fetch volume that endogenous slack punishes. The dual of the mechanism holds: coverage and volume cannot be decoupled with the signals a history-based predictor exposes. This is a stronger negative than a bare failure to capture, because the policy was designed against the exact bottleneck and the bottleneck absorbed it.
...
\item \textbf{The capture kill is bounded by its oracle decomposition.} The ladder prices the ingredient combinations in Table~\ref{tab:decomp}; a design outside that decomposition (for example, one that modifies the evictor to protect warmed objects) is out of scope, though the substitutes result argues against expecting gains from evictor coupling. The F7 kill was accepted only after the pre-registered fair-shot fallback (retraining the hazard model on the wide emission stream it is evaluated on), which raised the model's rank correlation without improving its capture.
