...
New file: `oppcert/ladder/r4_coverage.py`. Ceiling construction: a use of object X at t is "causally coverable" iff the
...
context, an implementation limit, not a property of the hypothesis. `oppcert/ladder/r4_coverage.py` gained `--wide-top-m`
...
New file: `oppcert/ladder/r5_emission.py`. Causal realization of the F5 ceiling: emit wide, value each candidate with
...
New file `oppcert/ladder/r6_placement.py`. F5's clairvoyant coverage+selection (funds only will-be-used objects), varying
...
**PENDING:** `oppcert/ladder/r6_placement.py` on wiki + c50, sweeping `--gamma {0.1, 0.25, 0.5}`.
...
the WIDE stream -- `oppcert/ladder/r6_hazard_model.py --top-m` wired in 77050b4 for exactly this).
...
New file `oppcert/ladder/r7_cgp.py`, pre-registration `prereg/r7_cgp.md` (both committed BEFORE the run). The one
...
New file `oppcert/domains/kvcache.py` (commits 29bbb69, 3d3dea3): the instrument ported to KV-cache WARMING on
