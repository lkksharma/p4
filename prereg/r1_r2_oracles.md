...
- Bar configs (winning strong-bar, from `docs/results_ledger.md`): wiki `markov2 τ=.05 k=1`;
  cluster50 `markov3 τ=.06 k=1`; cluster53 `markov2 τ=.20 k=1`.
- Budget: identical `pf_byte_rate` as the bar (computed exactly as `oppcert/sim/bootstrap.py` does —
...
`PFCache.run` calls `self.pf.suggest(o, cached, i)` after *every* request (`oppcert/sim/prefetch.py:329`). A
...
```python
NEVER = <as in oppcert.sim.prefetch>          # sentinel used by oracle_next
...
class JITSchedule:
    ...
    def reset(self):                            # MANDATORY: run() calls reset() (oppcert/sim/prefetch.py:220)
        self.pending = {}
...
2. **Head-of-line blocking is real.** The issue loop does `break`, not `continue`, when
   `tokens < xs` (`oppcert/sim/prefetch.py:333-334`). One oversized candidate at the front of the returned
...
With paired block-bootstrap CIs on F1_corridor and F4′ via the existing `oppcert/sim/bootstrap.py`
